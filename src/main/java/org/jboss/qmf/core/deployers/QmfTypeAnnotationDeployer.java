...
public class QmfTypeAnnotationDeployer extends GenericAnnotationDeployer
{
    private static final Logger log = Logger
            .getLogger(QmfTypeAnnotationDeployer.class);
    ...
    @Override
    protected void visitModule(DeploymentUnit deploymentUnit, Module module,
            GenericAnnotationResourceVisitor genericAnnotationResourceVisitor)
    {
        super.visitModule(deploymentUnit, module,
                genericAnnotationResourceVisitor);
        AnnotationEnvironment annotationEnvironment = genericAnnotationResourceVisitor
                .getEnv();
        createQmfTypesRegistrationMetadata(deploymentUnit,
                annotationEnvironment);
        createQmfAgentReferencesMetadata(deploymentUnit, annotationEnvironment);
    }

    private void createQmfAgentReferencesMetadata(
            ...
            AnnotationEnvironment annotationEnvironment)
    {
        ...
        Set<Element<QMFAgent, Field>> agentReferenceAnnotations = annotationEnvironment
                .classHasFieldAnnotatedWith(QMFAgent.class);
        ...
        deploymentUnit.addAttachment(QmfAgentReferencesMetaData.class,
                referencesMetadata);
    }

    private void createQmfTypesRegistrationMetadata(
            DeploymentUnit deploymentUnit,
            AnnotationEnvironment annotationEnvironment)
    {
        QmfRegisteredClassesMetaData metaData = new QmfRegisteredClassesMetaData();
        // create metadata for each class marked with QMFSeeAlso we need to
        // register
        Set<Element<QMFSeeAlso, Class<?>>> qmfSeeAlsoAnnotations = annotationEnvironment
                .classIsAnnotatedWith(QMFSeeAlso.class);
        for (Element<QMFSeeAlso, Class<?>> qmfSeeAlsoAnnotation : qmfSeeAlsoAnnotations)
        {
            // add the root class
            Class<?> annotatedClass = qmfSeeAlsoAnnotation
                    .getAnnotatedElement();
        ...
        // create metadata for each class marked with QMFType we need to
        // register
        Set<Element<QMFType, Class<?>>> qmfTypeAnnotations = annotationEnvironment
                .classIsAnnotatedWith(QMFType.class);
        for (Element<QMFType, Class<?>> qmfTypeAnnotation : qmfTypeAnnotations)
        {
            metaData.add(qmfTypeAnnotation.getAnnotatedElement());
        }
        // create metadata for each class marked with QMFEvent we need to
        // register
        Set<Element<QMFEvent, Class<?>>> qmfEventAnnotations = annotationEnvironment
                .classIsAnnotatedWith(QMFEvent.class);
    ...
    public void internalUndeploy(DeploymentUnit unit)
    {
        ...
        unit.removeAttachment(QmfRegisteredClassesMetaData.class);
        unit.removeAttachment(QmfAgentReferencesMetaData.class);
    }
}
